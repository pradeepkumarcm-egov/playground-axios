...
class SearchForm extends React.Component {
  ...
  componentDidMount = () => {
    ...
    instance.post('egov-mdms-service/v1/_search?tenantId=pb', {
      ...
      let dropDownLang = stateInfo.languages.map((lang) => {
        return {value: lang.value, key: lang.label};
      });
      let dropDownModule = stateInfo.localizationModules.map((mod) => {
        return {value: mod.value, key: mod.label};
      });
      this.setState({modules: dropDownModule, languages: dropDownLang});
    }).catch((error) => {
      console.log(error);
    });
  };
...
//  this.state.selectedModule="";
  ...
  handleSearch = () => {
    ...
    if (this.state.selectedModule==null || this.state.selectedModule=='' ){
  ...
  render() {
    ...
    return (<div>
      <Grid item="item" xs={12}>
        <Paper>
          <form>
            <h2>Search Localization Details</h2>
              ...
              <Select onChange={(e) => {
                ...
                <option aria-label="None" value=""/> {
                  this.state.languages.map((lan) => {
                    return <option value={lan.value} key={lan.key}>{lan.key}</option>
                  })
                }
              </Select>
            </FormControl>
            ...
            <Button onClick={this.handleReset} variant="contained" color="primary">
              Reset
            </Button>
            <br/><br/>
          </form>
